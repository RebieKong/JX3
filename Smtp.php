...
* @example
* $mail = new MySendMail();
* $mail->setServer("HOST", "ACCOUNT", "PWD"); //设置smtp服务器，普通连接方式
* $mail->setServer("HOST", "ACCOUNT", "PWD", 465, true); //设置smtp服务器，到服务器的SSL连接
* $mail->setFrom("XXXXX"); //设置发件人
* $mail->setReceiver("XXXXX"); //设置收件人(多个收件人传入数组)
* $mail->setCc("XXXX"); //设置抄送(多个抄送人传入数组)
* $mail->setBcc("XXXXX"); //设置秘密抄送(多个密送传入数组)
* $mail->addAttachment("XXXX"); //添加附件(多个附件传入数组)
...
class Smtp{
    ...
    * @return boolean
    */
    public function __construct($server, $username="", $password="", $port=25, $isSecurity=false) {
...
/**
    ...
    */
     ...
     * @return boolean
     */
    public function setEchoCommand($io=FALSE) {
    	$this->_echoComand = $io?TRUE:FALSE;
    	return true;
    }
    /**
    * 发送邮件
    ...
    */
     ...
     * @return boolean
     */
    protected function _setReceiver($to) {
    	$this->_to[] = $to;
    	return true;
    }
     ...
     * @return boolean
     */
    protected function _setBcc($bcc) {
    	$this->_bcc[] = $bcc;
    	return true;
    }
     ...
     * @return boolean
     */
    protected function _addAttachment($file) {
    	if(!file_exists($file)) {
